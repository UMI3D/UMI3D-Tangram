...
namespace umi3d.edk
{
    public class UMI3DAvatar : MonoBehaviour
    {
        ...
        public GameObject GLTFPivot = null;
        ...
        public void UpdateAvatar(NavigationRequestDto navigation)
        {
            ...
            switch (displayMode)
            {
                ...
                case AvatarDisplayMode.RiggedAvatarDisplay:
                    if (!this.defaultDisplay)
                    {
                        foreach (Transform child in this.anchor.transform)
                        {
                        ...
                        // size a la mano pour le moment. devrait utiliser la scale global du client. Donc 1.7 pour le PC actuellement. 
                        ...
                        {
                            GLTFPivot = new GameObject("GLTFPivot");
                            GLTFPivot.AddComponent<EmptyObject3D>();
                            GLTFPivot.transform.SetParent(transform);
                            GLTFPivot.transform.position = anchor.transform.position;
                            GLTFPivot.transform.localScale = new Vector3(
                             1.7f / avatarDto.ScaleScene.X,
                             1.7f / avatarDto.ScaleScene.Y,
                             1.7f / avatarDto.ScaleScene.Z);

                            UMI3DRiggedAvatar umi3dRiggedAvatar = Instantiate(this.GLTFAvatar, GLTFPivot.transform).GetComponent<UMI3DRiggedAvatar>();

                            if (umi3dRiggedAvatar)                  
                                umi3dRiggedAvatar.followingUser = this.user;

                            else
                                throw new Exception("UMI3DRiggedAvatar component not found");
                        }
                        else
                            throw new Exception("No valid representation");
        ...
        private void updateGobalPosition(NavigationRequestDto navigation)
        {
            ...
            {
                GLTFPivot.transform.position = anchor.transform.position;
                GLTFPivot.transform.rotation = Quaternion.Euler(new Vector3(GLTFPivot.transform.rotation.eulerAngles.x, anchor.transform.rotation.eulerAngles.y, GLTFPivot.transform.rotation.eulerAngles.z));
            }
        ...
        private void RenewAppearance()
        {
            ...
            foreach (string boneId in bonesToDelete)
            {
                ...
                Destroy(UMI3D.Scene.GetObject(umi3DAvatarBone.boneAnchorId).gameObject);
        ...
        private UMI3DAvatarBone InstanciateBone(BoneDto bone)
        {
            ...
            if (bone.type != BoneType.None)
            {
                ...
                GameObject Prefab;
                if (displayMode == AvatarDisplayMode.DynamicDisplay && listOfPrefabs.TryGetValue(bone.type, out Prefab))
                {
